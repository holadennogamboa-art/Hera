# LA COLA — un correo al día, sin decidir nada

> Nace de la [auditoría del 9 de septiembre](../fuentes/AUDITORIA-09-SEP.md): en 37 días
> salieron 7 correos, 0 seguimientos y 0 respuestas, con veinte contactos verificados
> esperando. El diagnóstico ya estaba escrito el 17 de agosto y se repitió dos veces.

**Versión de móvil, con los borradores dentro:**
https://claude.ai/code/artifact/a143569b-87be-41cd-8efa-8dbc3c26fa69

## Por qué existe este archivo

El cuello de botella no es la estrategia, ni los contactos, ni los textos: los tres están
resueltos. Es **pulsar enviar**. Y pulsar enviar falla porque cada día había que decidir
a quién, con qué ángulo y desde qué documento.

**Aquí no se decide nada.** Se abre, se manda el primero sin marca, y se marca.

Y desde el 10 de septiembre, **cada tarjeta lleva asunto y cuerpo escritos**. Antes solo
llevaba el ángulo, que es como no llevar nada: al abrirla seguías teniendo que redactar
desde cero, que es justo el punto donde se para. Los borradores son un punto de partida
para no empezar en blanco — lo que va entre corchetes lo rellenas tú, y lo que no suene a
ti se cambia.

**El correo a CANADA del que habla la auditoría está en tus documentos, no aquí.** Si lo
encuentras, usa el tuyo: será mejor que el mío.

---

## La cola

Uno al día. En orden. No se salta, no se elige, no se «prepara mejor».

| # | A quién | Correo | El ángulo | Hecho |
|---|---|---|---|---|
| 1 | **Alba Barneda** · CANADA, managing partner | `alba@canadacanada.com` | El dolor de los entregables: *«los entregables conforman el 95% del trabajo que hace postproducción»*, literal suyo. **Redactado desde agosto.** | ☐ |
| 2 | **Louis de Meckenheim** · Chopard | *(el del 3-ago)* | Seguimiento. 37 días de silencio. Dos líneas: sigo interesado, esto es nuevo. | ☐ |
| 3 | **Felix Mondino** · Iconoclast, Head of Print | `felix@iconoclastimage.tv` | Seguimiento del 18-ago. Es el único envío del método nuevo. | ☐ |
| 4 | **Maria Vives** · CANADA, head of photography | `maria@canadacanada.com` | Puerta B: fotografía y moda. Tu terreno. | ☐ |
| 5 | **Wendy** · Cuyana | *(el del lote de agosto)* | Seguimiento. | ☐ |
| 6 | **Callum Harrison** · CANADA, special projects | `callum@canadacanada.com` | Puerta C: su punto ciego con la IA. | ☐ |
| 7 | **Calvin Leung** · Cult Gaia | *(el del lote de agosto)* | Seguimiento. | ☐ |
| 8 | **Ilona Klaver** · Iconoclast | `ilona@iconoclastimage.tv` | Segunda puerta en Iconoclast. | ☐ |
| 9 | **Víctor Mata** · CANADA | `victor@canadacanada.com` | — | ☐ |
| 10 | **Bernard Amsellem** · Iconoclast | `bernard@iconoclast.tv` | Ojo: dominio distinto de Felix e Ilona. | ☐ |

### La decisión que bloquea el nº 1

Llevas cinco semanas aplazando una sola cosa: si el correo a CANADA sale **ya, con el
enlace a `/studio`**, o si espera a la pieza de versionado.

**Sale ya.** La pieza tiene brief del 19 de agosto, no existe, y el punto de corte que
pusiste tú mismo para ella era el 22. Esperar a una pieza que lleva tres semanas sin
empezar no es preparar mejor el envío: es el mecanismo de aplazarlo. `/studio` está en
línea, verificada, con tu nombre y tu trayectoria — que era justo el agujero que tenía la
web. Ya sirve de puerta.

Si la pieza se hace después, se manda como segundo contacto. Eso es un motivo para volver
a escribir, no una razón para no escribir hoy.

---

**Antes del primero, verifica las direcciones en el directorio publicado por ellos.** La
auditoría las da abreviadas (`maria@`, `callum@`) y yo he completado el dominio. Un rebote
gasta un día de cola.

### No tocar todavía
- **Carolina Herrera** — Eduardo Lozano y Jodie Chan rebotaron. Puig valida destinatarios.
  La vía real es LinkedIn o un ex-compañero tuyo.
- **Tcherassi · Johanna Ortiz · Agua Bendita · El Palacio de Hierro** — falta verificar
  el dominio.
- **Los 8 de joyería europea** de `EMAILS-MANANA.md`: los textos son del 9 de agosto y
  vienen de otro posicionamiento. Habría que reescribirlos, y reescribir no es enviar.

---

## Las dos reglas que salen de lo que ya funcionó

1. **Escribir arriba.** 8 correos a directores de marketing y creativos → 0 respuestas.
   1 correo a una CEO → lo reenvió por dentro, y de ahí salieron la demo, el equipo, el
   brief, el NDA y la final del pitch. Escribe a quien tiene el reflejo de reenviar.
2. **Leer directorios, no adivinar dominios.** 0 de 8 adivinando · 2 de 2 leyendo lo que
   la productora publica. Las marcas esconden a su gente; las productoras la publican.

## Cómo se cuenta

Cada envío se anota el mismo día en la bitácora:

```
fecha,B4,T7,outreach,"Correo a <nombre> · <empresa>",enviado,15,"<ángulo en seis palabras>"
```

Un «no» también se anota. Un silencio de catorce días pide seguimiento, no otro contacto
nuevo: **cero seguimientos en 37 días** es el agujero más caro de la auditoría.

## Dos arreglos de dos minutos que siguen pendientes

Están en el móvil, no necesitan Mac, y llevan meses en rojo:

- [ ] **El nombre en Instagram.** Sigue «THE_EYEHERA». Tu nombre no aparece en ninguna
      parte del perfil. Quien te busca por Denno Gamboa no te encuentra.
- [ ] **El correo en la bio.** Sigue «Inquiries: DM». Un director de casting no manda un DM.

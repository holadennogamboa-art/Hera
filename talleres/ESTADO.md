# ESTADO — fuente única

> Se regenera cada **viernes en el bloque de Dirección**. Todo lo que no esté aquí, no
> existe para el sistema. Última actualización: **20 de septiembre de 2026**.

## La regla que hace que esto funcione

> **El hilo es donde se piensa. El repositorio es donde queda.**

Cada conversación empieza sin memoria de las otras. Lo único que viaja entre ellas es lo
que está escrito aquí. Al terminar cualquier hilo —de la novela, de la serie, del auditor—
lo decidido se escribe en el repo. Sin eso, cada semana se abren dos sistemas paralelos que
dejan de hablarse.

---

## Los diez frentes

| # | Frente | Estado | h/sem | Siguiente paso concreto |
|---|--------|--------|-------|-------------------------|
| [T1](T1-golpe-de-estado.md) | **Golpe de estado** | Novela cerrada, corrigiendo | 4,5 | Párrafo 12 del capítulo 7 |
| [T2](T2-verde-agua.md) | **Verde agua** | Taller montado, sin abrir | 6 | El umbral, paso 1 de 15 |
| [T3](T3-hera.md) | **HERA · marca** | Cadencia | 3 | Calendario editorial de la semana |
| [T4](T4-verder.md) | **Auditor GEO** | Funciona, sin clientes | 3 | Enviar Peñalver |
| [T5](T5-skin-lab.md) | **HERA Skin Lab** | **1ª conversión completa** | 1,5 | Preguntar a vicky por qué no descargó |
| [T6](T6-kierck.md) | **Kierck** | Biblia en construcción | 1,5 | Fijar la primera frase de ella |
| [T7](T7-encargos.md) | **Encargos y outreach** | **Atascado en enviar** | 1,5 | [La cola](t7-encargos/COLA-DE-ENVIO.md), correo nº 1 |
| [T8](T8-direccion.md) | **Dirección** | — | 1,5 | Revisión del viernes |
| [T9](T9-envios-revistas.md) | **Envíos a revistas** | **Kenyon enviado ✔** | 1,5 | Nada hasta marzo. No mirar. |
| [T10](T10-bus-beasts.md) | **The Bus Beasts** | **EN PAUSA** | 0 | Nada. Cajón. |
| [La ronda](LA-RONDA.md) | **B4, todos los días** | — | 6 | Reparte T3, T4 y T5 |

**Total: 30 h/semana en 20 bloques de 90 minutos.**

---

## Lo urgente, por orden

0. **Rotar la clave de Gemini.** Lleva expuesta desde agosto: siete semanas. Diez minutos
   y se acaba. Es lo único de esta lista que puede costarte dinero mientras duermes.
0b. **Preparar los cinco que quedan de la cola**, en el Mac y con Enviados abierto. No se
   trata de mandarlos: se trata de dejarlos escritos enteros, que es lo que hizo que los
   otros cinco salieran.
1. **Enviar Peñalver.** El paquete está hecho. Solo hay que mandarlo.
2. **Subir el ZIP y mandar Txirimiri.**
3. **Día 8: seguimiento de SI SI BAGELS** si no han contestado.
4. **Rotar la clave de Gemini.** Expuesta desde agosto.
5. **Decidir qué va a Kenyon.** La ventana corre.

## Lo que espera a otros (no son tareas tuyas)

- Mikel, sobre citar su frase.
- SI SI BAGELS y No Classica, sobre el auditor.
- El registro de propiedad intelectual de Madrid, en trámite.

## Las cinco correcciones pendientes en las biblias

Detalladas en [`fuentes/biblias/REVISION-05-SEP.md`](fuentes/biblias/REVISION-05-SEP.md):

1. «Like a player» → **«Like a Prayer»** en el Anexo A de Verde agua.
2. Escribir que **la canción se corta con la última inhalación**.
3. Subir el sistema **BYN → color → BYN** a §6 de la columna vertebral.
4. Subir a §3 que **Carlos dormía en la habitación de al lado**.
5. Volcar en la biblia de Kierck lo que solo está en el índice: biografía política, primera
   frase de ella, confidente francés, cibercafé, créditos.

---

## Los números

| | |
|---|---|
| Párrafos de novela | 11 de 132 |
| Pasos del umbral escritos | 0 de 15 |
| Personas ajenas que han renderizado | 2 confirmadas · 1 conversión completa |
| Renders totales | 35 · 34 con IA · 0 errores |
| Correos comerciales fuera | **12 · respuestas 0** |
| Seguimientos enviados | **1** (Felix Mondino, 16-sep) — antes eran cero |
| Cola de envío | **5 de 10 fuera** · los 5 que quedan exigen buscar en Enviados |
| Pasos del umbral escritos | **0 de 15** · la página no se ha abierto |
| Contactos verificados sin usar | **~20** |
| Último envío con constancia | **16 de septiembre · Ilona Klaver, Iconoclast** |
| Negocios auditados | 27 · con contacto verificado |
| Clientes que pagan | 0 |
| Hitos de la semana | 1 de 5 |

## Lo que la auditoría del 9-sep destapó

La [auditoría](fuentes/AUDITORIA-09-SEP.md) mide el outreach de Phoenix Studio a
productoras y marcas: 37 días, 7 correos, 0 seguimientos, 0 respuestas, con veinte
contactos verificados esperando y los textos ya escritos.

**Y encontré la causa, que la auditoría no nombra: ese frente no tenía bloque.** La
ronda diaria reserva quince minutos para «un correo», pero ese correo estaba asignado al
Auditor GEO —cafés de Malasaña—, no a CANADA ni a Iconoclast. No existía ningún
`OUTREACH-LOG.md` en el repositorio. El trabajo se hacía en documentos sueltos que el
sistema no miraba, y por eso cada día había que decidir de nuevo a quién escribir.

Arreglado así: el outreach es ahora **T7**, con una cola numerada de diez envíos donde no
se decide nada, y la ronda apunta a ella.

## Lo que no se mueve

**Verde agua tiene el mayor reparto de horas y cero escenas escritas.** Ahora tiene biblia
cerrada, material de campo transcrito, canción decidida, sistema de color resuelto y un
taller de 15 pasos montado con sus propias grabaciones. Ya no queda nada entre él y la
página.
